...
func TestGoogleProviderValidateGroup(t *testing.T) {
	...
	assert.Equal(t, true, p.ValidateSession(&SessionState{"michael.bland@gsa.gov"}))
	p.GroupValidator = func(email string) bool {
		return email != "michael.bland@gsa.gov"
	}
	assert.Equal(t, false, p.ValidateSession(&SessionState{Email: "michael.bland@gsa.gov"}))
}

func TestGoogleProviderWithoutValidateSession(t *testing.T) {
	p := newGoogleProvider()
	assert.Equal(t, true, p.ValidateSession(&SessionState{Email: "michael.bland@gsa.gov"}))
}
